...
namespace simple_form
{
   ...
   void simple_form_002::on_layout(::draw2d::graphics_pointer & pgraphics)
   {
      ...
      m_pstill->set_window_pos(e_zorder_top, iLeft, y, 200, iHeight, SWP_SHOWWINDOW);

      y += 30;

      m_pedit->set_window_pos(e_zorder_top, iLeft-30, y, 200, iHeight, SWP_SHOWWINDOW);

      y += 30;

      m_pcombobox->set_window_pos(e_zorder_top, iLeft, y, 200, iHeight, SWP_SHOWWINDOW);

      y += 30;

      m_pbutton->set_window_pos(e_zorder_top, iLeft-30, y, 200, iHeight, SWP_SHOWWINDOW);
