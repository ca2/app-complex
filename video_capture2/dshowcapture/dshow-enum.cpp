...
static bool EnumPinCaps(IPin *pin, EnumCapsCallback callback, void *param)
{
	...
	if (SUCCEEDED(hr)) {
		...
		for (int i = 0; i < count; i++) {
			MediaTypePtr mt;
			hr = config->GetStreamCaps(i, &mt, caps.get_data());
			if (SUCCEEDED(hr))
				if (!callback(param, *mt, caps.get_data()))
					break;
		}
		...
		const AM_MEDIA_TYPE &mt, const byte *data)
{
	...
	if (vscc) {
		...
		info.granularityCX = maximum(vscc->OutputGranularityX, 1);
		info.granularityCY = maximum(vscc->OutputGranularityY, 1);
	} else {
...
static inline void ClampToGranularity(::i32 &val, int minVal, int granularity)
{
		...
		const AM_MEDIA_TYPE &mt, const byte *capData)
{
	...
	if (!data.found || data.bestVal > totalVal) {
		if (channelsVal == 0) {
			::i32 channels = data.config.channels;
		...
		if (sampleRateVal == 0) {
			...
			ClampToGranularity((::i32&)wfex->nSamplesPerSec,
					info.minSampleRate,
