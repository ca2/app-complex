...
STDMETHODIMP OutputPin::EnumMediaTypes(IEnumMediaTypes **ppEnum)
{
	...
	*ppEnum = ___new OutputEnumMediaTypes(this);
	if (!*ppEnum)
...
OutputFilter::OutputFilter(const PinOutputInfo &info)
	: refCount (0),
	  state    (State_Stopped),
	  pin      (__allocate< OutputPin >(this, info)),
...
STDMETHODIMP OutputFilter::EnumPins(IEnumPins **ppEnum)
{
	...
	*ppEnum = ___new OutputEnumPins(this, nullptr);
...
{
	*ppEnum = ___new OutputEnumPins(filter, this);
	return (*ppEnum == nullptr) ? E_OUTOFMEMORY : NOERROR;
}
...
{
	*ppEnum = ___new OutputEnumMediaTypes(pin);
	return (*ppEnum == nullptr) ? E_OUTOFMEMORY : NOERROR;
}
