...
#pragma once
	...
	#define DSHOWCAPTURE_EXPORT _CLASS_DECL_EXPORT
#else
...
namespace DShow {
	...
	typedef void fnVideoProc(const VideoConfig &config,
			unsigned char *data, size_t size,
			long long startTime, long long stopTime);

   typedef fnVideoProc * VideoProc;

	typedef void fnAudioProc(const AudioConfig &config,
			unsigned char *data, size_t size,
			long long startTime, long long stopTime);

   typedef fnAudioProc * AudioProc;
