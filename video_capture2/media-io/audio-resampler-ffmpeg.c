...
struct audio_resampler {
	...
	u8             *output_buffer[MAX_AV_PLANES];
	u64            output_layout;
...
static inline u64 convert_speaker_layout(enum speaker_layout layout)
{
	switch (layout) {
		 ...
		 const u8 *const input[], u32 in_frames)
{
	...
	i64 delay = swr_get_delay(context, rs->input_freq);
	int estimated = (int)av_rescale_rnd(
			delay + (i64)in_frames,
			(i64)rs->output_freq, (i64)rs->input_freq,
			AV_ROUND_UP);

	*ts_offset = (u64)swr_get_delay(context, 1000000000);
	...
	for (u32 i = 0; i < rs->output_planes; i++)
