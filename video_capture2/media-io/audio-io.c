...
static inline double ts_to_frames(const audio_t *audio, u64 ts)
{
...
static i64 ts_diff_frames(const audio_t *audio, u64 ts1, u64 ts2)
{
	double diff = ts_to_frames(audio, ts1) - ts_to_frames(audio, ts2);
	return (i64)positive_round(diff);
}

static i64 ts_diff_bytes(const audio_t *audio, u64 ts1, u64 ts2)
{
	return ts_diff_frames(audio, ts1, ts2) * (i64)audio->block_size;
}

/* ------------------------------------------------------------------------- */

static inline u64 min_uint64(u64 a, u64 b)
{
	return a < b ? a : b;
}
...
static inline void do_audio_output(struct audio_output *audio,
		size_t mix_idx, u64 timestamp, u32 frames)
{
...
static void *audio_thread(void *param)
{
	...
	u64 samples = 0;
	u64 start_time = os_gettime_ns();
	u64 prev_time = start_time;
	u64 audio_time = prev_time;
	u32 audio_wait_time =
		(u32)(audio_frames_to_ns(rate, AUDIO_OUTPUT_FRAMES) /
				1000000);
...
int audio_output_open(audio_t **audio, struct audio_output_info *info)
{
	...
	::memcpy_dup(&out->info, info, sizeof(struct audio_output_info));
...
u32 audio_output_get_sample_rate(const audio_t *audio)
{
