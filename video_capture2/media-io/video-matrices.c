		...
		float range_min[3], float range_max[3])
{
	...
	for (size_t i = 0; i < NUM_FORMATS; i++) {
		...
		::memcpy_dup(matrix, format_info[i].matrix[full_range],
				sizeof(float) * 16);

		if (range == VIDEO_RANGE_FULL) {
			if (range_min)
				::memcpy_dup(range_min, full_min, sizeof(float) * 3);
			if (range_max)
				::memcpy_dup(range_max, full_max, sizeof(float) * 3);
			return true;
		}

		if (range_min)
			::memcpy_dup(range_min, format_info[i].float_range_min,
					sizeof(float) * 3);

		if (range_max)
			::memcpy_dup(range_max, format_info[i].float_range_max,
					sizeof(float) * 3);
