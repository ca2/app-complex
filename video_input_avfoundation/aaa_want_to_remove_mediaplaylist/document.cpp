...
namespace mediaplaylist
{
   ...
   bool document::on_new_document()
   {
      ...
      auto pplaylist = allocateø multimedia_playlist::veriwell::playlist();
      ...
      if (pstraNew != nullptr)
      {
         ...
         if(pstraNew->get_count() > 0)
         {
            ...
            for (::collection::index i = 0; i < m_straOnAddPlay.get_count(); i++)
            {
               ...
               for (::collection::index j = 0; j < pstraNew->get_count(); j++)
               {
            ...
            for (::collection::index i = 0; i < m_straOnAddQueue.get_count(); i++)
            {
...
processed_new:
   ...
   bool document::play(info * pinfo)
   {
      ...
      ::collection::index iSong = pinfo->m_iCurrentSong;
...
retry0:
   ...
   bool document::RemoveSong(::collection::index iSong, ::pointer<::user::impact>pimpact, bool bUpdate)
   {
...
//         newName["url"] = strName;
//         newName["xmledit"] = allocateø ::memory_file(*memfileEdit);
////         newName["xmledit"].cast < ::memory_file >()->release();
//         newName["xml"] = allocateø ::memory_file(*memfileXml);
