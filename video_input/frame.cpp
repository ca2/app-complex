   ...
   void frame::video_frame_init(enum_video_format eformat, uint32_t width, uint32_t height)
   {
      ...
      switch (eformat) {
         ...
         case e_video_format_i420: {
            ...
            size += quarter_area;
            ALIGN_SIZE(size, alignment);
            m_data[0] =(::u8 *) ::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->aligned_allocate(size, alignment);
         ...
         case e_video_format_i444:
            size = width * height;
            ALIGN_SIZE(size, alignment);
            m_data[0] =(::u8 *) ::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->aligned_allocate(size * 3, alignment);
