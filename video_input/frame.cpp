...
frame::~frame()
{
   ...
   //{
      ///if (frame) {
   ::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->free(m_data[0]);
         //bfree(frame);
      ///}
   //}
   ...
   void frame::video_frame_init(enum_video_format eformat, uint32_t width, uint32_t height)
   {
      ...
      switch (eformat) {
         ...
         case e_video_format_i420: {
            ...
            size += quarter_area;
            ALIGN_SIZE(size, alignment);
            m_data[0] =(::u8 *) ::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->aligned_allocate(size, nullptr, alignment);
         ...
         case e_video_format_i444:
            size = width * height;
            ALIGN_SIZE(size, alignment);
            m_data[0] =(::u8 *) ::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->aligned_allocate(size * 3, nullptr, alignment);
...
//::acme::get()->m_pheapmanagement->memory(::heap::e_memory_secondary)->aligned_allocate(size, alignment);
//}
//
//memory_copy(source
...
//}
//
///* resamples/remixes __new audio to the designated main audio output format */
