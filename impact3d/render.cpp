...
#include <sstream>
#include <string>
#include "third/glm/glm/geometric.hpp"
...
namespace app_complex_impact3d
{
   ...
   void render::initialize(::object * pobject)
   {
      ...
      m_pcontext = m_psystem->m_paurasystem->get_gpu()->create_context();
       ...
       m_pcontext->m_pbuffer && ::is_ok(m_pcontext->m_pbuffer->m_pimage))
      {
         
         {
...

            {
               ...
               double dElapsed = m_durationStart.elapsed().floating_second().m_d;
   ...
   void render::load_obj(const ::payload & payloadFile, ::array<glm::vec4> & vertices, ::array<glm::vec3> & normals, ::array<GLushort> & elements)
   {

      auto preader = ::object::m_pcontext->m_papexcontext->file().get_reader(payloadFile);
      ...
      while (preader->read_string(strLine))
      {

         if (strLine.begins_eat("v "))
         {

            ::std::istringstream s(strLine.c_str());
