...
int video_output_open(video_t **video, struct video_output_info *info)
{
	...
	::memcpy_dup(&out->info, info, sizeof(struct video_output_info));
	out->frame_time = (u64)(1000000000.0 * (double)info->fps_den /
		(double)info->fps_num);
		...
		int count, u64 timestamp)
{
	...
	if (video->available_frames == 0) {
		...
		::memcpy_dup(frame, &cfi->frame, sizeof(*frame));
...
u64 video_output_get_frame_time(const video_t *video)
{
...
u32 video_output_get_width(const video_t *video)
{
	return video ? video->info.width : 0;
}

u32 video_output_get_height(const video_t *video)
{
...
u32 video_output_get_skipped_frames(const video_t *video)
{
	return video->skipped_frames;
}

u32 video_output_get_total_frames(const video_t *video)
{
