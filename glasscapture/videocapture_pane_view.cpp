...
namespace videocapture
{
   ...
   void pane_view::on_message_create(::message::message * pmessage)
   {
      ...
      set_tab("videocapture", MAIN_IMPACT);
   ...
   void pane_view::on_change_cur_sel()
   {
      ...
      if(get_view_id() == GCOM_IMPACT
            ...
            || stra.contains(__str((int)GCOM_IMPACT))
            ...
            || stra.contains(__str((int)MAIN_SWITCHER_IMPACT)))
      {
         ...
         if(get_pane_by_id(FONTSEL_IMPACT)->m_pholder != nullptr)
         {
         ...
         if(get_view_id() == GCOM_IMPACT)
         {
            ...
            m_pviewLast = dynamic_cast < impact_base * > (get_pane_by_id(GCOM_IMPACT)->m_pholder->get_child_by_id("videocapture_gcom"));

         }
         else if(get_view_id() == MAIN_IMPACT)
         {

            m_pviewLast = dynamic_cast < impact_base * > (get_pane_by_id(MAIN_IMPACT)->m_pholder->get_child_by_id("videocapture_view"));
            m_pviewLastBilbo = dynamic_cast < view * > (get_pane_by_id(MAIN_IMPACT)->m_pholder->get_child_by_id("videocapture_view"));

         }
         else if (get_view_id() == MAIN_SWITCHER_IMPACT)
         {

            m_pviewLast = dynamic_cast < impact_base * > (get_pane_by_id(MAIN_SWITCHER_IMPACT)->m_pholder->get_child_by_id("videocapture_view"));
            m_pviewLastBilbo = dynamic_cast < view * > (get_pane_by_id(MAIN_SWITCHER_IMPACT)->m_pholder->get_child_by_id("videocapture_view"));
      ...
      else if (get_view_id() == MENU_IMPACT)
      {
         ...
         if (m_pviewLastBilbo != nullptr)
         {
            ...
            for (auto & bilbo : m_pviewLastBilbo->m_bilboa)
            {
      ...
      else if (get_view_id() == FONTSEL_IMPACT)
      {
         ...
         __pointer(FONTSEL_IMPACT)->m_pholder->get_child_by_id("font_list_view");
...
//         m_prollspf = pview->get_child_by_id("roll_spf");
      ...
      case MAIN_IMPACT:
      {
         __pointer(::user::document) pdocument =  papplication->m_ptemplateHelloMultiverseView->open_document_file(get_application(), nullptr,true,pimpactdata->m_pplaceholder);
      ...
      case PaneViewHelloMultiverseSwitcher:
      {
         ...
         papplication->m_ptemplateHelloMultiverseSwitcher->open_document_file(get_application(), nullptr,true,pimpactdata->m_pplaceholder);
      ...
      {
         papplication->m_ptemplateHelloMultiverseFontSel->open_document_file(get_application(), nullptr,true,pimpactdata->m_pplaceholder);
            ...
            if (::str::begins_eat_ci(strCheck, "bilbo"))
            {
                     ...
                     && !pevent->m_context.is_source(::e_source_sync))
               {
                  ...
                  if (m_pviewLastBilbo != nullptr)
                  {
                     ...
                     m_pviewLastBilbo->m_bilboa[iCheck - 1].m_bNew = pevent->m_puie->_001GetCheck() == ::check_checked;

                     m_pviewLastBilbo->defer_update_bilbo();
